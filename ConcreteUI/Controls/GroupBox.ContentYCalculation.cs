...
namespace ConcreteUI.Controls
{
    ...
    {
        public sealed class TextYCalculation : AbstractCalculation
        {
            private readonly WeakReference<GroupBox> _reference;
            private readonly int _offset;

            public TextYCalculation(WeakReference<GroupBox> reference, int offset = 0)
            {
                _reference = reference;
                _offset = offset;
            }

            public TextYCalculation(GroupBox element, int offset = 0) : this(new WeakReference<GroupBox>(element), offset)
            {
            }
            ...
            private sealed class CalculationContext : ICalculationContext
            {
                ...
                public unsafe int DoCalc(Rect* pPageRect, int dependedValue)
                {
                    if (_calculated)
                        return _value;
                    int value = _element.GetTextBaseYCore(dependedValue) + _offset;
                    _value = value;
                    _calculated = true;
                    return value;
                }
