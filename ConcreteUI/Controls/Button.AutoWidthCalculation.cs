        ...
        public sealed class AutoWidthCalculation : AbstractCalculation
        {
            ...
            private sealed class CalculationContext : ICalculationContext
            {
                ...
                public unsafe int DoCalc(Rect* pPageRect, int dependedValue)
                {
                    if (_calculated)
                        return _value;
                    int value = DoCalc(_element, dependedValue) + UIConstants.ButtonExtraSpace;
                    _value = value;
                    _calculated = true;
                    return value;
                }

                private int DoCalc(Button element, int dependedValue)
                {
                    string text = element._text;
                    if (string.IsNullOrEmpty(text))
                        return MathHelper.Max(_minWidth, 0);
                    DWriteTextLayout layout = TextFormatUtils.CreateTextLayout(text, TextAlignment.MiddleCenter, element._fontSize);
                    if (layout is null)
                        return MathHelper.Max(_minWidth, 0);
